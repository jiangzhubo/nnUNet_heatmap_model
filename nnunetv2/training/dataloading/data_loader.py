...
class nnUNetDataLoader(DataLoader):
    def __init__(
        ...
        self.data_shape, self.seg_shape, self.heatmap_shape = self.determine_shapes()
    ...
    def determine_shapes(self):
        # load one case
        data, seg, seg_prev, heatmap, properties = self._data.load_case(
            self._data.identifiers[0]
        )
        ...
        heatmap_shape = (
            (self.batch_size, 1, *self.patch_size) if heatmap is not None else None
        )
        self.heatmap_in_dataset = heatmap is not None
        return data_shape, seg_shape, heatmap_shape
    ...
    def generate_train_batch(self):
        ...
        if self.heatmap_in_dataset:
            heatmap_all = np.zeros(self.heatmap_shape, dtype=np.float32)
        ...
        for j, i in enumerate(selected_keys):
            ...
            data, seg, seg_prev, heatmap, properties = self._data.load_case(i)
            ...
            if self.heatmap_in_dataset:
                if heatmap is None:
                    heatmap_cropped = np.zeros(self.patch_size, dtype=np.float32)
                else:
                    heatmap_cropped = crop_and_pad_nd(heatmap, bbox, 0)
                heatmap_all[j, 0] = heatmap_cropped

        if self.patch_size_was_2d:
            data_all = data_all[:, :, 0]
            seg_all = seg_all[:, :, 0]
            if self.heatmap_in_dataset:
                heatmap_all = heatmap_all[:, :, 0]
        ...
        if self.transforms is not None:
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    ...
                    del segs, images
            if self.heatmap_in_dataset:
                heatmap_all = torch.from_numpy(heatmap_all).float()
                return {
                    "data": data_all,
                    "target": seg_all,
                    "heatmap": heatmap_all,
